...
function nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth) {
  ...
  let currentLoss = percentLossByMonth;
  let moneySaved = 0;

  while (currentOldPrice + moneySaved < currentNewPrice) {
    months++;

    // Increase depreciation every 2 months (at months 2, 4, 6 ...)
    if (months % 2 === 0) {
      currentLoss += 0.5;
    }

    // Depreciate prices at end of month
    currentOldPrice *= (1 - currentLoss / 100);
    currentNewPrice *= (1 - currentLoss / 100);

    // Save money for this month
    moneySaved += savingperMonth;
  }

  // leftover = money saved + old car value - new car price
  let leftover = currentOldPrice + moneySaved - currentNewPrice;
